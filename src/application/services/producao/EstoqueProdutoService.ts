...
import {
  EstoqueProdutoBuscarTodosResponseDTO,
  EstoqueProdutoItemDTO,
} from "@/application/dtos/producao/EstoqueProduto/EstoqueProdutoBuscarTodosResponseDTO";
...
export class EstoqueProdutoService {
  ...
  async buscarTodos(
    ...
    const dadosEnriquecidos = await Promise.all(
      response.dados.map(async (estoque) => {
        ...
        const sigla = await this.medidaRepository.buscarSigla(
  ...
  async atualizar(dto: EstoqueProdutoAtualizarDTO): Promise<void> {
    const estoqueExistente = await this.estoqueProdutoRepository.buscarPorId(
      dto.id
    );
  ...
  async verificarEDebitarEstoque(
    produtoId: string,
    quantidadeNecessaria: number
  ): Promise<void> {
    const lotes = await this.estoqueProdutoRepository.buscarPorProdutoOrdenado(
      produtoId
    );

    const totalDisponivel = lotes.reduce(
      (acc, lote) => acc + (lote.quantidade ?? 0),
      0
    );
    ...
    for (const lote of lotes) {
      ...
      await this.estoqueProdutoRepository.debitarQuantidade(
        lote.id,
        quantidadeADebitar
      );
